...
import { deleteCardHandlerUpdateCustomerObj, deleteCardHandlerCustomerId } from '../const/PaymentHandlerConst';
...
test.serial('Get delete card handler data ', async (t) => {
  const result = await paymentHandler.deleteCardHandler(deleteCardHandlerUpdateCustomerObj, deleteCardHandlerCustomerId);
  t.is(result.actions[0].action, 'setCustomType');
